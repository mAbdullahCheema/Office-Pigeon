...
const trustedCountryHeaderNames = [
  'cf-ipcountry',
  'x-vercel-ip-country',
  'x-country-code',
...
const buildPakistanRestrictedPage = () => `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
...
async function startServer() {
  ...
  app.get('/api/region-offer', (req, res) => {
    ...
    res.json({
      showPakistanOffer,
      region: showPakistanOffer ? 'PK' : null
    });
  });
  ...
  app.get(['/pakistan', '/pakistan/'], (req, res, next) => {
    ...
    res.status(200).type('html').send(buildPakistanRestrictedPage());
  });
