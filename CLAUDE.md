...
- **Done:**
  ...
  - **Phase 3 frontend SSR + SEO core: DONE.** All 13 marketing pages ported to `app/(site)/<route>` Server Components (reuse `src/views/*` as client islands via `SiteChrome` context). Per-route metadata (SEO-02), JSON-LD Org/LocalBusiness/Service/FAQ/Breadcrumb (SEO-04), sitemap/robots (SEO-05) — all verified in view-source. `next build` (26 routes) green; SPA still green. Pattern in 04-HANDOFF §6b.
- **Not done / next:**
  - **Phase 3 backend (remaining):** API 1:1 parity (Next Route Handlers for the Express endpoints Next lacks: chat, pip-lead, contact-submission, package-inquiry, preview-leads, region-offer, admin/*, public/previews, preview file serving+banner), Pakistan gating→`middleware.ts`, security headers (SEC-02) + trust-proxy fix (SEC-01), og:image, then the **boot cutover** (`dist/server.cjs` shim → Next standalone; ⚠️ needs owner Hostinger entry-file + Node-22 change; Express kept as tagged rollback). Test locally → push.
