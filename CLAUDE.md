...
- **Done:**
  ...
  - **Phase 3 API parity (most): DONE.** Ported 1:1 from `server.ts` (reuse `lib/*` + new `lib/server/{env,formUtils,previews,adminAuth,pakistanPage}`, `lib/geo/country`): contact-submission, package-inquiry, region-offer, public/previews, admin/{config,me,previews,previews/[slug],previews/[slug]/status}. **Pakistan geo-gating** via `middleware.ts`. `next build` + SPA lint green.
- **Not done / next:**
  - **Finish Phase 3 backend:** preview file serving + banner (`/previews/:slug/*`) + `/api/preview-leads`; security headers (SEC-02) + trust-proxy (SEC-01); og:image; then the **boot cutover** (`dist/server.cjs` shim → Next standalone; ⚠️ needs owner Hostinger entry-file + Node-22 change; Express kept as tagged rollback). Test locally → push.
