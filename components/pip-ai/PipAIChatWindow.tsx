...
export default function PipAIChatWindow({ isOpen, onClose, onPageChange }: { isOpen: boolean; onClose: () => void; onPageChange?: (page: string) => void }) {
  ...
  return (
    <motion.div
      ...
      className={`fixed bottom-4 right-4 z-[9999] flex h-[min(680px,calc(100vh-2rem))] w-[calc(100vw-2rem)] max-w-[410px] flex-col overflow-hidden rounded-[28px] border border-cyan-100 bg-white/95 shadow-2xl shadow-cyan-950/15 backdrop-blur-xl max-[640px]:right-3 max-[640px]:bottom-4 max-[640px]:h-[calc(100vh-2rem)] max-[640px]:w-[calc(100vw-1.5rem)] sm:bottom-6 sm:right-6 ${
