...
## Current Focus
**Phase 3 in progress — frontend SSR + SEO core + most API parity DONE.** All 13 pages SSR'd; metadata/JSON-LD/sitemap/robots live. **API parity so far (Next Route Handlers, 1:1 with `server.ts`, reuse `lib/*`):** contact-submission, package-inquiry, region-offer, admin/{config,me,previews,previews/[slug],previews/[slug]/status}, public/previews — plus the pre-existing pip/*, admin/reindex, elevenlabs/search. **Pakistan geo-gating** done via `middleware.ts`. **Remaining Phase 3:** preview file serving + preview-leads, security headers + trust-proxy, og:image, then the **boot cutover** (needs owner Hostinger action). Express still live until cutover.

## Next Up (start here next session)
1. **Finish API parity:** **preview file serving + banner injection** (`/previews/:slug/*` → Next catch-all route: disk read from `previewDirCandidates()`, `injectPreviewHtml`/`buildPreviewInjection`/`buildExpiredPreviewPage` ported into `lib/server/previews.ts`, status gating, `X-Robots-Tag: noindex` + cache headers) and **`/api/preview-leads`** (rate-limited Supabase insert; the banner posts here). Legacy `/api/chat` + `/api/pip-lead` are NOT used by the SPA (it uses `/api/pip/chat` + `/api/pip/lead`, both exist) — port only if wanted for completeness.
2. **Security headers** via `next.config` `headers()` (CSP/HSTS/X-CTO/frame-ancestors/Referrer-Policy/Permissions-Policy, SEC-02); fix `trust proxy` (SEC-01); admin allowlist env-only (SEC-04).
3. **og:image** 1200×630 branded → wire into OpenGraph in `pageMetadata`/root.
4. **Boot cutover (last):** build copies `.next/standalone/*`+`.next/static`+`public/`+`previews/` into `dist/`; `dist/server.cjs` → thin shim booting Next standalone. **Tag Express rollback first.** ⚠️ Needs owner Hostinger action (entry/startup file + Node 22) — assistant hands exact steps. Test locally → push.
5. **RESP-08:** re-measure `/websites` CLS post-cutover. Phase 0 leftovers (ESLint/Prettier, Playwright). Search Console TXT (owner) + sitemap submit after green signal.
...
| SEC-01 | trust proxy spoofable | 3 | ⬜ | fix at cutover (Next reads x-forwarded-for via lib/geo getClientIp) |
